...
class ModCog(commands.Cog, name="Moderation"):
    ...
    async def mute(self, ctx, member: discord.Member):
        ...
        if muted_role is None:
            ...
            perms.update(read_messages=True, create_instant_invite=True, stream=True, speak=True, connect=True, use_voice_activation=True, view_channel=True, read_message_history=True)
            muted_role = await guild.create_role(name=name)
            await muted_role.edit(permissions=perms)
        if member.roles.__contains__(muted_role):
        ...
        embed.set_thumbnail(url=f'{member.avatar_url}')
        embed.add_field(name="User Muted:", value=f"<@!{member.id}>")
        await ctx.send(embed=embed)
    ...
    async def poll(self, ctx, *, content: str):
        ...
        embed = discord.Embed(title=f"Poll: (by {ctx.message.author.display_name})", color=0x9b59b6)
        embed.set_thumbnail(url=f'{ctx.message.author.avatar_url}')
        embed.add_field(name="Question:", value=f"{content}", inline=True)

        role = discord.utils.get(ctx.guild.roles, name="Poll Pings")

        msg = await channel.send(f"{role.mention}", embed=embed)
        await msg.add_reaction("✅")
        await msg.add_reaction("❌")


@commands.command(brief="Set channels for system messages.")
async def set_system(ctx, id: int, type: str):
...
@set_system.error
async def system_handler(ctx, error):
